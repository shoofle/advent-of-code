...
(defn check-match [ticker sue]
  (every? (fn [[quality value]] (= (get ticker quality) value)) sue)
  )

(defn kwify [s] (keyword (subs s 0 (dec (count s)))))

"Sue 1: cars: 9, akitas: 3, goldfish: 0"
(defn munge-sue [whole-line]
  (let [[raw-sue raw-n k1 v1 k2 v2 k3 v3] (s/split whole-line #" ")
        sue (str raw-sue " " raw-n)
        qualities {(kwify k1) (read-string v1)
                   (kwify k2) (read-string v2)
                   (kwify k3) (read-string v3)}
        ]
    {sue qualities}
    )
  )

(munge-sue "Sue 1: cars: 9, akitas: 3, goldfish: 0")

;part 1
(filter (fn [[s v]] (if (check-match ticker v) s false)) all-sues)

;part 2
(defn check-match-2 [ticker sue]
  (every? (fn [[quality value]]
            (cond (#{:cats :trees} quality) (> value (get ticker quality))
                  (#{:pomeranians :goldfish} quality) (< value (get ticker quality))
                  :else (= value (get ticker quality))))
          sue)
  )

(filter (fn [[s v]] (if (check-match-2 ticker v) s false)) all-sues)

; put all the input in here
(def all-sues (reduce merge (map munge-sue (s/split-lines "Sue 1: cars: 9, akitas: 3, goldfish: 0
